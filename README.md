...
1. 请自行搜索下载安装32位的Java运行库（即使你正在使用64位操作系统）
2. 下载测试版zip压缩包
3. 解压缩
...
程序未在其他机器上测试，windows vista以上的系统应该都能运行，软件界面需要微软雅黑字体，windows xp系统没有该字体，需要自行安装。
...
自己写这个程序的初衷是尽可能地让其帮助自己快速产生内容，此程序目的不在于控制文档外观、组织文档、兼容众多文件格式，因此程序功能更多侧重在文字组织、分析以及灵感产生上。
...
* 输入法设置为中文模式时，可通过alt+字母键a-z在关键词列表中选词，也可以在输入法中按下某个字母键后再补敲回车选词；输入法为小写字母模式或处于关闭状态时，直接按下关键词列表中对应的字母键就可以选词，按一次选词，按两次可直接将所选词汇插入正文
* 选中关键词按下数字键1/2可分别插入人名中的名和姓（目前仅支持中译后的外国人名）
* 点击文件篓右键菜单里的“显示选择框”可以多选文件，选择文件后按下alt+上下左右方向键可以快速移动文档
* ……
